...
Item {
    ...
    Node {
        ...
        // FixedJoint.positionA/positionB/orientationA/orientationB reach e.g.
        // PxFixedJointCreate()'s PX_CHECK_AND_RETURN_NULL(localFrame.isSane(), ...) --
        // compiled out (PX_CHECKED, Debug-only) -- with no validation of our own either.
        StaticRigidBody {
            id: jointAnchor
        ...
        FixedJoint {
            bodyA: jointAnchor
            bodyB: jointDynamicBody
            positionA: Qt.vector3d(NaN, 0, 0)
        }
    ...
    PhysicsTestCase {
        name: "invalidinput_teleportAndJoint"
        goalReached: invalidShapeWorld.frameCount > 5
    }
